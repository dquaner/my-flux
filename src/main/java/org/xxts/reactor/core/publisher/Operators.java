 ...
 * <br>
 * 一个辅助类，支持 "Operator"(算子) ：写入、处理无操作的订阅、
 * 验证请求数量并限制并发添加操作到 Long.MAX_VALUE，{@link Subscription#request(long)} 处理的通用值。
 *
 * <p>
...
public abstract class Operators {
	 ...
	 * algorithms need to hand out(分发) a subscription, replace this with a singleton
	 * subscription because there is no standard way to tell if a Subscription is cancelled
	 * or not otherwise.
	 * <br>
	 * 表示一个已取消的 Subscription 单例，它不应该泄露给客户端，因为它表示一个 terminal 状态。
	 * 如果算法需要分发订阅，请将其替换为此单例订阅，因为没有标准的方法来判断订阅是否被取消。
	 *
	 ...
	 * {@link reactor.core.publisher.Flux#transform(Function)}, {@link reactor.core.publisher.Mono#transform(Function)},
	 * {@link reactor.core.publisher.Hooks#onEachOperator(Function)} and {@link reactor.core.publisher.Hooks#onLastOperator(Function)},
	...
	static final <R> Function<Context, Context> discardLocalAdapter(Class<R> type, Consumer<? super R> discardHook) {
		...
		return ctx -> {
			Consumer<Object> consumer = ctx.getOrDefault(reactor.core.publisher.Hooks.KEY_ON_DISCARD, null);
			if (consumer == null) {
				return ctx.put(reactor.core.publisher.Hooks.KEY_ON_DISCARD, safeConsumer);
			}
			else {
				return ctx.put(reactor.core.publisher.Hooks.KEY_ON_DISCARD, safeConsumer.andThen(consumer));
			}
		};
	}

	/**
	 * Utility method to activate the onDiscard feature (see {@link reactor.core.publisher.Flux#doOnDiscard(Class, Consumer)})
	 * in a target {@link Context}. Prefer using the {@link reactor.core.publisher.Flux} API, and reserve this for
	 * testing purposes.
	 *
	...
	public static final Context enableOnDiscard(@Nullable Context target, Consumer<?> discardConsumer) {
		...
		if (target == null) {
			return Context.of(reactor.core.publisher.Hooks.KEY_ON_DISCARD, discardConsumer);
		}
		return target.put(reactor.core.publisher.Hooks.KEY_ON_DISCARD, discardConsumer);
	}
	...
	public static <T> void onDiscard(@Nullable T element, Context context) {
		Consumer<Object> hook = context.getOrDefault(reactor.core.publisher.Hooks.KEY_ON_DISCARD, null);
   ...
   */
	 ...
	 * <p>
	 * If no hook is registered for {@link reactor.core.publisher.Hooks#onErrorDropped(Consumer)}, the dropped
	...
	public static void onErrorDropped(Throwable e, Context context) {
		Consumer<? super Throwable> hook = context.getOrDefault(reactor.core.publisher.Hooks.KEY_ON_ERROR_DROPPED,null);
		if (hook == null) {
			hook = reactor.core.publisher.Hooks.onErrorDroppedHook;
		}
		if (hook == null) {
	 ...
	 * <p>
	 * If no hook is registered for {@link reactor.core.publisher.Hooks#onNextDropped(Consumer)}, the dropped
	...
	public static <T> void onNextDropped(T t, Context context) {
		...
		Consumer<Object> hook = context.getOrDefault(reactor.core.publisher.Hooks.KEY_ON_NEXT_DROPPED, null);
		if (hook == null) {
			hook = reactor.core.publisher.Hooks.onNextDroppedHook;
		}
		if (hook != null) {
			hook.accept(t);
		}
	...
	static final reactor.core.publisher.OnNextFailureStrategy onNextErrorStrategy(Context context) {
		reactor.core.publisher.OnNextFailureStrategy strategy = null;

		BiFunction<? super Throwable, Object, ? extends Throwable> fn = context.getOrDefault(
				reactor.core.publisher.OnNextFailureStrategy.KEY_ON_NEXT_ERROR_STRATEGY, null);
		if (fn instanceof reactor.core.publisher.OnNextFailureStrategy) {
			strategy = (reactor.core.publisher.OnNextFailureStrategy) fn;
		} else if (fn != null) {
			strategy = new reactor.core.publisher.OnNextFailureStrategy.LambdaOnNextErrorStrategy(fn);
		}

		if (strategy == null) strategy = reactor.core.publisher.Hooks.onNextErrorHook;
		if (strategy == null) strategy = reactor.core.publisher.OnNextFailureStrategy.STOP;
		return strategy;
	}
	...
	/**
	 * Find the {@link reactor.core.publisher.OnNextFailureStrategy} to apply to the calling operator (which could be a local
	 ...
	 * @param context The most significant {@link Context} in which to look for an {@link reactor.core.publisher.OnNextFailureStrategy}.
	...
	public static <T> Throwable onNextError(@Nullable T value, Throwable error, Context context,
		...
		reactor.core.publisher.OnNextFailureStrategy strategy = onNextErrorStrategy(context);
	...
	/**
	 * Find the {@link reactor.core.publisher.OnNextFailureStrategy} to apply to the calling async operator (which could be
	...
	/**
	 * Applies the hooks registered with {@link reactor.core.publisher.Hooks#onLastOperator} and returns
	...
	public static <T> CorePublisher<T> onLastAssembly(CorePublisher<T> source) {
		Function<Publisher, Publisher> hook = reactor.core.publisher.Hooks.onLastOperatorHook;
		if (hook == null) {
			return source;
		}
	...
	public static <T> CorePublisher<T> toFluxOrMono(Publisher<T> publisher) {
		if (publisher instanceof reactor.core.publisher.Mono) {
			return reactor.core.publisher.Mono.fromDirect(publisher);
		}
	...
	public static RuntimeException onRejectedExecution(Throwable original,
		...
		if (context.hasKey(reactor.core.publisher.Hooks.KEY_ON_REJECTED_EXECUTION)) {
			context = context.put(reactor.core.publisher.Hooks.KEY_ON_OPERATOR_ERROR, context.get(reactor.core.publisher.Hooks.KEY_ON_REJECTED_EXECUTION));
		}
	 ...
	 * <br>
	 * 如果 subscription 不是 {@link #cancelledSubscription()}，则自动终止订阅：
	 * 取消订阅并将 field 设置为单例 {@link #cancelledSubscription()}。
	 *
	 * @param <F> the instance type containing the field
	 * @param field the field accessor
	 * @param instance the parent instance
	 ...
	 * <br> 评估请求是否严格为正，否则 {@link #reportBadRequest(long)}
	 *
